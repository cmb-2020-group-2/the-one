/*
 * Copyright 2010 Aalto University, ComNet
 * Released under GPLv3. See LICENSE.txt for details.
 */
package movement;

import core.Coord;
import core.Settings;
import core.SimClock;
import input.WKTReader;
import movement.map.DijkstraPathFinder;
import movement.map.MapNode;
import movement.map.SimMap;

import java.io.File;
import java.util.LinkedList;
import java.util.List;

/**
 * @author Frans Ekman
 */
public class BreakActivityMovement extends MapBasedMovement
        implements SwitchableMovement {

    private static final int WALKING_TO_BREAK_MODE = 0;
    private static final int BREAK_MODE = 1;

    public static final String BREAK_LENGTH_SETTING = "breakLength";
    public static final String NR_OF_TABLE_SETTING = "nrOfTables";

    public static final String TABLE_FILE_SETTING = "tablesFile";

    private static int nrOfTables = 5;

    private int mode;
    private int[] breakLength;
    private double specificWaitTime;
    private int startedBreakTime;
    private boolean ready;
    private DijkstraPathFinder pathFinder;

    private List<Coord> allTables;

    private Coord lastWaypoint;
    private Coord tableLocation;

    /**
     * Creates a new instance of EveningActivityMovement
     *
     * @param settings
     */
    public BreakActivityMovement(Settings settings) {
        super(settings);
        super.backAllowed = false;
        pathFinder = new DijkstraPathFinder(null);
        mode = WALKING_TO_BREAK_MODE;

        nrOfTables = settings.getInt(NR_OF_TABLE_SETTING);

        breakLength = settings.getCsvInts(BREAK_LENGTH_SETTING);

        startedBreakTime = -1;

        String tablesFile = null;
        try {
            tablesFile = settings.getSetting(TABLE_FILE_SETTING);
        } catch (Throwable t) {
            // Do nothing;
        }

        if (tablesFile == null) {
            MapNode[] mapNodes = (MapNode[])getMap().getNodes().
                    toArray(new MapNode[0]);
            int officeIndex = rng.nextInt(mapNodes.length - 1) /
                    (mapNodes.length/ nrOfTables);
            tableLocation = mapNodes[officeIndex].getLocation().clone();
        } else {
            try {
                allTables = new LinkedList<Coord>();
                List<Coord> locationsRead = (new WKTReader()).
                        readPoints(new File(tablesFile));
                for (Coord coord : locationsRead) {
                    SimMap map = getMap();
                    Coord offset = map.getOffset();
                    // mirror points if map data is mirrored
                    if (map.isMirrored()) {
                        coord.setLocation(coord.getX(), -coord.getY());
                    }
                    coord.translate(offset.getX(), offset.getY());
                    allTables.add(coord);
                }
                tableLocation = allTables.get(
                        rng.nextInt(allTables.size())).clone();
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    }

    /**
     * Creates a new instance of EveningActivityMovement from a prototype
     *
     * @param proto
     */
    public BreakActivityMovement(BreakActivityMovement proto) {
        super(proto);
        this.breakLength = proto.breakLength;
        startedBreakTime = -1;
        this.pathFinder = proto.pathFinder;
        this.mode = proto.mode;

        if (proto.allTables == null) {
            MapNode[] mapNodes = (MapNode[])getMap().getNodes().
                    toArray(new MapNode[0]);
            int officeIndex = rng.nextInt(mapNodes.length - 1) /
                    (mapNodes.length/ nrOfTables);
            tableLocation = mapNodes[officeIndex].getLocation().clone();
        } else {
            this.allTables = proto.allTables;
            tableLocation = allTables.get(
                    rng.nextInt(allTables.size())).clone();
        }
    }

    @Override
    public Coord getInitialLocation() {
        double x = rng.nextDouble() * getMaxX();
        double y = rng.nextDouble() * getMaxY();
        Coord c = new Coord(x,y);

        this.lastWaypoint = c;
        return c.clone();
    }

    @Override
    public Path getPath() {
        if (mode == WALKING_TO_BREAK_MODE) {
            // Try to find to the office
            SimMap map = super.getMap();
            if (map == null) {
                return null;
            }
            MapNode thisNode = map.getNodeByCoord(lastWaypoint);
            MapNode destinationNode = map.getNodeByCoord(tableLocation);
            List<MapNode> nodes = pathFinder.getShortestPath(thisNode,
                    destinationNode);
            Path path = new Path(generateSpeed());
            for (MapNode node : nodes) {
                path.addWaypoint(node.getLocation());
            }

            lastWaypoint = tableLocation.clone();
            mode = BREAK_MODE;
            return path;
        }
        if (startedBreakTime == -1) {
            startedBreakTime = SimClock.getIntTime();
        }
        if (SimClock.getIntTime() >= startedBreakTime + specificWaitTime) {
            Path path = new Path(1);
            path.addWaypoint(lastWaypoint.clone());
            ready = true;
            return path;
        }
        return null;
    }

    @Override
    protected double generateWaitTime() {
        int lower = breakLength[0];
        int upper = breakLength[1];

        specificWaitTime = (upper - lower) *
                rng.nextDouble() + lower;
        return specificWaitTime;
    }

    @Override
    public MapBasedMovement replicate() {
        return new BreakActivityMovement(this);
    }

    /**
     * @see SwitchableMovement
     */
    public Coord getLastLocation() {
        return lastWaypoint.clone();
    }

    /**
     * @see SwitchableMovement
     */
    public boolean isReady() {
        return ready;
    }

    /**
     * @see SwitchableMovement
     */
    public void setLocation(Coord lastWaypoint) {
        this.lastWaypoint = lastWaypoint.clone();
        startedBreakTime = -1;
        ready = false;
        mode = WALKING_TO_BREAK_MODE;
    }

    /**
     * @return The location of the office
     */
    public Coord getTableLocation() {
        return tableLocation.clone();
    }

    public List<Coord> getAllTables() { return this.allTables; }
}
